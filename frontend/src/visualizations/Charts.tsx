...
function GenericTimeChart({ result }: { result: SimulationResult }) {
    ...
    const stateKeys = result.rounds.length > 0
        ...
        : [];
    const colors = ["#06b6d4", "#d946ef", "#8b5cf6", "#f59e0b", "#10b981", "#ef4444", "#3b82f6"];
...
/* ── Colonel Blotto ────────────────────────────────────────────────────── */
function BlottoChart({ result }: { result: SimulationResult }) {
    const bfRates = result.summary.battlefield_win_rates_p1 as number[];
    return (
        <Plot
            data={[{
                x: bfRates.map((_, i) => `BF ${i + 1}`), y: bfRates, type: "bar",
                marker: { color: bfRates.map((r) => r > 0.5 ? "#06b6d4" : r < 0.5 ? "#ef4444" : "#f59e0b") },
            }]}
            layout={{ ...darkLayout, xaxis: { ...darkAxis, title: "Battlefield" }, yaxis: { ...darkAxis, title: "P1 Win Rate", range: [0, 1] }, height: 300, showlegend: false }}
            config={plotConfig} style={{ width: "100%" }}
        />
    );
}
...
function AuctionChart({ result }: { result: SimulationResult }) {
    ...
    return (
        <Plot
            data={[
                { x: rounds, y: result.rounds.map((r) => r.state.revenue), type: "scatter", mode: "lines", name: "Revenue", line: { color: "#f59e0b", width: 2 } },
                { x: rounds, y: result.rounds.map((r) => r.state.winner_surplus), type: "scatter", mode: "lines", name: "Winner Surplus", line: { color: "#06b6d4", width: 2 } },
            ]}
...
/* ── Network Contagion ─────────────────────────────────────────────────── */
function NetworkChart({ result }: { result: SimulationResult }) {
    const rounds = result.rounds.map((r) => r.round_num);
    return (
        <Plot
            data={[
                { x: rounds, y: result.rounds.map((r) => r.state.adoption_rate), type: "scatter", mode: "lines", name: "Adoption Rate", line: { color: "#10b981", width: 2 }, fill: "tozeroy", fillcolor: "rgba(16,185,129,0.1)" },
                { x: rounds, y: result.rounds.map((r) => r.state.new_adopters), type: "bar", name: "New Adopters", marker: { color: "rgba(139,92,246,0.4)" }, yaxis: "y2" },
            ]}
            layout={{ ...darkLayout, xaxis: { ...darkAxis, title: "Round" }, yaxis: { ...darkAxis, title: "Adoption Rate", range: [0, 1] }, yaxis2: { ...darkAxis, title: "New Adopters", overlaying: "y", side: "right" }, height: 320 }}
            config={plotConfig} style={{ width: "100%" }}
        />
    );
}
...
/* ── War of Attrition ──────────────────────────────────────────────────── */
function WarOfAttritionChart({ result }: { result: SimulationResult }) {
    const rounds = result.rounds.map((r) => r.round_num);
    return (
        <Plot
            data={[
                { x: rounds, y: result.rounds.map((r) => r.state.avg_duration), type: "scatter", mode: "lines", name: "Avg Duration", line: { color: "#ef4444", width: 2 }, fill: "tozeroy", fillcolor: "rgba(239,68,68,0.08)" },
                { x: rounds, y: result.rounds.map((r) => r.state.p1_win_rate), type: "scatter", mode: "lines", name: "P1 Win Rate", line: { color: "#06b6d4", width: 2, dash: "dot" } },
            ]}
            layout={{ ...darkLayout, xaxis: { ...darkAxis, title: "Contest" }, yaxis: { ...darkAxis, title: "Value" }, height: 300 }}
            config={plotConfig} style={{ width: "100%" }}
        />
    );
}
...
/* ── Market Entry ──────────────────────────────────────────────────────── */
function MarketEntryChart({ result }: { result: SimulationResult }) {
    const rounds = result.rounds.map((r) => r.round_num);
    return (
        <Plot
            data={[
                { x: rounds, y: result.rounds.map((r) => r.state.n_entrants), type: "bar", name: "Entrants", marker: { color: "rgba(139,92,246,0.4)" } },
                { x: rounds, y: result.rounds.map((r) => r.state.profit_per_firm), type: "scatter", mode: "lines", name: "Profit/Firm", line: { color: "#06b6d4", width: 2 }, yaxis: "y2" },
            ]}
            layout={{ ...darkLayout, xaxis: { ...darkAxis, title: "Period" }, yaxis: { ...darkAxis, title: "Entrants" }, yaxis2: { ...darkAxis, title: "Profit", overlaying: "y", side: "right" }, height: 300 }}
            config={plotConfig} style={{ width: "100%" }}
        />
    );
}
...
function NegotiationChart({ result }: { result: SimulationResult }) {
    const rounds = result.rounds.map((r) => r.round_num);
    return (
        <Plot
            data={[
                { x: rounds, y: result.rounds.map((r) => r.state.allocated_ratio), type: "scatter", mode: "lines", name: "Items Allocated", line: { color: "#10b981", width: 2.5 }, fill: "tozeroy", fillcolor: "rgba(16,185,129,0.08)" },
                { x: rounds, y: result.rounds.map((r) => r.state.efficiency), type: "scatter", mode: "lines", name: "Allocation Efficiency", line: { color: "#f59e0b", width: 2 } },
            ]}
            layout={{ ...darkLayout, xaxis: { ...darkAxis, title: "Round" }, yaxis: { ...darkAxis, title: "Rate", range: [0, 1.05] }, height: 300 }}
            config={plotConfig} style={{ width: "100%" }}
        />
    );
}
...
export const CHART_MAP: Record<string, React.ComponentType<{ result: SimulationResult }>> = {
    ...
    colonel_blotto: BlottoChart,
    auction_mechanisms: AuctionChart,
    network_contagion: NetworkChart,
    ...
    ess_module: ESSChart,
    war_of_attrition: WarOfAttritionChart,
